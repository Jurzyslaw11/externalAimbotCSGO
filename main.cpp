...
int main()
{
	...
	while (true)
	{
		...
		const auto& localTeam = memory.Read<std::uintptr_t >(client + offset::m_iTeamNum);
		...
		for (auto i = 1; i <= 32; ++i)
		{
			...
			if (memory.Read<std::uintptr_t>(player + offset::m_iTeamNum) == localTeam)
				continue;
